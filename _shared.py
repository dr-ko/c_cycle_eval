...
def _get_set():
    common_set_file = os.getenv('common_settings_cres')
    co_settings = json.load(open(common_set_file))
    ...
    for _md in _models:
        ...
        if _md == 'obs':
            ...
            if _get_model_time_series:
                ...
                datVar = obs_dict[_whichVar]['model_var']
                if _whichVar in ["tasp", "taspn", "taspneg5"]:
                    datfile = os.path.join(
                        top_dir + 'Models/harmonized_' + reso,
                        'lpj_' + _scn + '_' + datVar + '_mean_' + syear + '-' +
...
def _fit_least_square(_xDat_tr,
    ...
    if method == 'quad':
        ...
        if _intercept:
            res_lsq_robust = least_squares(_quadfun_res, [0, 0, 0],
                                           jac='3-point',
                                           ftol=1e-15,
                                           xtol=1e-15,
                                           method='trf',
...
def _zonal_correlation(data_dict, var_dict, zonal_set):
    ...
    windowSize = int(np.round(bandsize_corr / (_latint * 2)))
...
def _plot_mm_norm_mean_r(_sp, allmodels_r, zonal_set, fig_set):
    ...
    # zmm_ens = np.nanmedian(allmodels_z, axis=1)
    # zmm_ens_low = np.nanpercentile(allmodels_z, perc_range[0], axis=1)
    # zmm_ens_high = np.nanpercentile(allmodels_z, perc_range[1], axis=1)
    zmm_ens = np.nanmean(allmodels_z, axis=1)
    zmm_ens_std = np.nanstd(allmodels_z, axis=1)
    r_mmod = _inverse_fisher_z(zmm_ens)
    # r_mmod_std_low = _inverse_fisher_z(zmm_ens_low)
    # r_mmod_std_hi = _inverse_fisher_z(zmm_ens_high)
