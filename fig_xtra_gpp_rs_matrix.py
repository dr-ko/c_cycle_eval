...
file_list = []
models = []
for fi_le in os.listdir(top_dir + 'Observation/FLUXCOM/fluxcom_all_rs_memb/long_term_mean'):
    if fi_le.endswith(".nc"):
        print(os.path.join(fi_le))
        file_list = np.append(file_list, os.path.join(top_dir + 'Observation/FLUXCOM/fluxcom_all_rs_memb/long_term_mean', fi_le))
        models = np.append(models, fi_le.split('.8daily')[0])
print(models)
# kera
