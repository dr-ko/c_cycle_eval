...
for a_set,the_set in set_s.items():
    ...
    #---------------------------------------------------------------------------
    file_list = []
    for fi_le in os.listdir(top_dir + 'Observation/FLUXCOM/fluxcom_all_all_memb/long_term_mean'):
        if fi_le.endswith(".nc"):
            print(os.path.join(fi_le))
            file_list = np.append(file_list, os.path.join(top_dir + 'Observation/FLUXCOM/fluxcom_all_all_memb/long_term_mean', fi_le))
