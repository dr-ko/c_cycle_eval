...
for a_set,the_set in set_s.items():
    ...
    gpp_c = np.nanmedian(xr.open_dataset(os.path.join(top_dir,'Observation/tau_carvalhais2014_cube/gpp_Jung2011_' + the_set['carv_name'] + '_full.nc'), decode_times=False)['gpp'].values.reshape(-1, 360, 720), axis = 0)
    cSoil_c = np.nanmedian(xr.open_dataset(os.path.join(top_dir,'Observation/tau_carvalhais2014_cube/c_soil_Carvalhais2014_' + the_set['carv_name'] + '_full.nc'), decode_times=False)['c_soil'].values.reshape(-1, 360, 720), axis = 0)
    c_veg_c = np.nanmedian(xr.open_dataset(os.path.join(top_dir,'Observation/tau_carvalhais2014_cube/c_veg_SaatchiThurner_' + the_set['carv_name'] + '_full.nc'), decode_times=False)['c_veg'].values.reshape(-1, 360, 720), axis = 0)
    ...
    #---------------------------------------------------------------------------
    file_list = []
    for fi_le in os.listdir(top_dir + 'Observation/FLUXCOM/gpp_ensembles'):
        if fi_le.endswith(".nc"):
            print(os.path.join(fi_le))
            file_list = np.append(file_list, os.path.join(top_dir + 'Observation/FLUXCOM/gpp_ensembles', fi_le))
    # file_list = []
    # for fi_le in os.listdir(top_dir + 'Observation/FLUXCOM/gpp_rs_meteo'):
    #     if fi_le.endswith(".nc"):
    #         print(os.path.join(fi_le))
    #         file_list = np.append(file_list, os.path.join(top_dir + 'Observation/FLUXCOM/gpp_rs_meteo', fi_le))
    ...
    gpps = np.ones((len(file_list) + 1, 360, 720)) * np.nan
