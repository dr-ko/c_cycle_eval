...
out_dir = os.path.join(top_dir + 'Observation/tau_extended_cube_only_fan/')
...
#---------------------------------------------------------------------------
# gpp_c = np.nanmedian(xr.open_dataset(os.path.join(top_dir,'Observation/tau_carvalhais2014_cube/gpp_Jung2011_full.nc'), decode_times=False)['gpp'].values.reshape(-1, 360, 720), axis = 0)
# cSoil_c = np.nanmedian(xr.open_dataset(os.path.join(top_dir,'Observation/tau_carvalhais2014_cube/c_soil_Carvalhais2014_full.nc'), decode_times=False)['c_soil'].values.reshape(-1, 360, 720), axis = 0)
# c_veg_c = np.nanmedian(xr.open_dataset(os.path.join(top_dir,'Observation/tau_carvalhais2014_cube/c_veg_SaatchiThurner_full.nc'), decode_times=False)['c_veg'].values.reshape(-1, 360, 720), axis = 0)
...
c_vegs_use = [0, 1, 2, 3]
c_vegs = np.ones((len(c_vegs_use),360, 720)) * np.nan
