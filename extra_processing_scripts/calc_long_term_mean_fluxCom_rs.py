...
mainDir_t = './fluxcom_all_rs_memb'
odir = './fluxcom_all_rs_memb/long_term_mean'
...
for _mlm in mlms:
    for _varb in varbs:
        ofile_n = '{_varbi}.{_mlmi}.8daily.rs-long_term_mean.nc'.format(_varbi=_varb, _mlmi=_mlm)
        ofile = os.path.join(odir, ofile_n)
        odat = np.ones((nYrs, 360,720))
        yr_ind = 0
        for _yr in range(syr, eyr+1):
            # print (file)
            infile_n = '{_varbi}.{_mlmi}.8daily.{_yri}.rs-ltMean.nc'.format(_varbi=_varb, _mlmi=_mlm, _yri=str(_yr))
