...
def get_fitted_data(_xDat,
    ...
    for _tr in range(len(_aridity_bounds) - 1):
        ...
        mod_taspm_tmp = np.ma.masked_inside(mod_arI, tas1, tas2).mask
        _yDat_tr = _yDat[mod_taspm_tmp]
        _xDat_tr = _xDat[mod_taspm_tmp]
...
all_mod_dat_taspm = _get_data('taspm',
                                          co_settings,
                                          _co_mask=all_mask)
#---------------------------------------------------------------------------
...
mod_taspm_0 = all_mod_dat_taspm['lpj']
    ...
    for c_soil in c_soils_sel:
        ...
        for c_veg in c_veg_sel:
            ...
            for gpp in gpp_sel:
                ...
                mod_taspm = np.copy(mod_taspm_0)
                mod_pr = np.copy(mod_pr_0)
                mod_pr, mod_gpp, mod_arI, mod_taspm, mod_tau_c = _apply_common_mask_g(
                    mod_pr, mod_gpp, arI, mod_taspm, mod_tau_c)
