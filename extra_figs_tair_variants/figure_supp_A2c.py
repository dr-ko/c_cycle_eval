...
mod_taspn = all_mod_dat_taspn['lpj']
mod_pr = all_mod_dat_pr['lpj']
# plt.figure()
# plt.scatter(mod_taspn, mod_wai)
# plt.show()
x_tit_taspn = obs_dict['taspn']['title'] + ' (' + obs_dict['taspn']['unit'] + ')'
...
mod_wai, mod_arI, mod_taspn, mod_pr = _apply_common_mask_g(
    mod_wai, mod_arI, mod_taspn, mod_pr)
# print(mod_gpp.shape)
...
for _tr in range(len(_aridity_bounds) - 1):
    ...
    _xDat_tr = mod_taspn[mod_taspn_tmp]
    _yDat_tr = mod_pr[mod_taspn_tmp]
    _zDat_tr = mod_wai[mod_taspn_tmp]
    mevery = 1
...
plt.xlabel(x_tit_taspn, ha='center', fontsize=ax_fs * 0.9)
