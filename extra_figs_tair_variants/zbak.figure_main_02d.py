...
all_mod_dat_tasp50 = _get_data('tasp50', co_settings, _co_mask=all_mask)
...
all_mod_dat_tasp50['obs'] = all_mod_dat_tasp50['lpj']
...
for rel in rela_tions.keys():
    ...
    for row_m in range(nmodels):
        ...
        for _tr in range(len(_aridity_bounds) - 1):
            tasp501 = _aridity_bounds[_tr]
            tasp502 = _aridity_bounds[_tr + 1]
            mod_x_tmp = np.ma.masked_inside(mod_arI, tasp501, tasp502).mask
