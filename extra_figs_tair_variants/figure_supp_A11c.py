...
def get_fitted_data(_xDat,
    ...
    for _tr in range(len(_aridity_bounds) - 1):
        ...
        mod_taspn_tmp = np.ma.masked_inside(mod_arI, taspn1, taspn2).mask
        _yDat_tr = _yDat[mod_taspn_tmp]
        _xDat_tr = _xDat[mod_taspn_tmp]
...
all_mod_dat_taspn = _get_data('taspn',
                                          co_settings,
                                          _co_mask=all_mask)
#---------------------------------------------------------------------------
...
mod_taspn_0 = all_mod_dat_taspn['lpj']
    ...
    for c_soil in c_soils_sel:
        ...
        for c_veg in c_veg_sel:
            ...
            for gpp in gpp_sel:
                ...
                mod_taspn = np.copy(mod_taspn_0)
                mod_pr = np.copy(mod_pr_0)
                mod_pr, mod_gpp, mod_arI, mod_taspn, mod_tau_c = _apply_common_mask_g(
                    mod_pr, mod_gpp, arI, mod_taspn, mod_tau_c)
