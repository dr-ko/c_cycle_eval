...
all_mod_dat_taspm = _get_data('taspm', co_settings, _co_mask=all_mask)
...
all_mod_dat_taspm['obs'] = all_mod_dat_taspm['lpj']
...
for rel in rela_tions.keys():
    ...
    for row_m in range(nmodels):
        ...
        for _tr in range(len(_aridity_bounds) - 1):
            taspm1 = _aridity_bounds[_tr]
            taspm2 = _aridity_bounds[_tr + 1]
            mod_x_tmp = np.ma.masked_inside(mod_arI, taspm1, taspm2).mask
