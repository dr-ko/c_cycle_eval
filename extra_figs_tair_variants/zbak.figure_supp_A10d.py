...
def get_fitted_data(_xDat,
    ...
    for _tr in range(len(_aridity_bounds) - 1):
        ...
        mod_tasp50_tmp = np.ma.masked_inside(mod_arI, tas1, tas2).mask
        _yDat_tr = _yDat[mod_tasp50_tmp]
        _xDat_tr = _xDat[mod_tasp50_tmp]
...
all_mod_dat_tasp50 = _get_data('tasp50',
                                          co_settings,
                                          _co_mask=all_mask)
#---------------------------------------------------------------------------
...
mod_tasp50_0 = all_mod_dat_tasp50['lpj']
...
data_sel = {
    ...
    'c_soil': {
        "c_soils_sel": np.copy(c_soils),
        "c_veg_sel": np.copy(np.nanmedian(c_vegs,axis=0).reshape(-1, 360, 720)),
        "gpp_sel": np.copy(np.nanmedian(gpps,axis=0).reshape(-1, 360, 720)),
        "spStart": 5,
        "title": obs_dict['c_soil']['title']
    },
    'cVeg': {
        "c_soils_sel": np.copy(np.nanmedian(c_soils,axis=0).reshape(-1, 360, 720)),
        "c_veg_sel": np.copy(c_vegs),
        "gpp_sel": np.copy(np.nanmedian(gpps,axis=0).reshape(-1, 360, 720)),
        "spStart": 9,
    ...
    for c_soil in c_soils_sel:
        ...
        for c_veg in c_veg_sel:
            ...
            for gpp in gpp_sel:
                ...
                mod_tasp50 = np.copy(mod_tasp50_0)
                mod_pr = np.copy(mod_pr_0)
                mod_pr, mod_gpp, mod_arI, mod_tasp50, mod_tau_c = _apply_common_mask_g(
                    mod_pr, mod_gpp, arI, mod_tasp50, mod_tau_c)
