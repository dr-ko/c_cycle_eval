...
relS = 'taspn-mrso,taspn-ra,taspn-rh,taspn-gpp,mrso-ra,mrso-rh,mrso-gpp'.split(',')
nRel = len(relS)
...
for row_m in range(nmodels):
    ...
    mod_taspn = all_mod_dat_taspn['lpj'] #- 273.15
    mod_mrso = all_mod_dat_mrso[row_mod]
    mod_mrro = all_mod_dat_mrro[row_mod]
    mod_evapotrans = all_mod_dat_evapotrans[row_mod]
    mod_mrso, mod_gpp, mod_arI, mod_taspn, mod_ra, mod_rh, mod_evapotrans, mod_mrro = _apply_common_mask_g(
        mod_mrso, mod_gpp, arI, mod_taspn, mod_ra, mod_rh, mod_evapotrans, mod_mrro)
    for reStr in relS:
        _intercept=True
        varX = reStr.split('-')[0]
        varY = reStr.split('-')[1]
