...
file_list = []
models = []
for fi_le in os.listdir(top_dir + 'Observation/FLUXCOM/fluxcom_all_mte_memb/long_term_mean'):
    if fi_le.endswith(".nc"):
        print(os.path.join(fi_le))
        file_list = np.append(file_list, os.path.join(top_dir + 'Observation/FLUXCOM/fluxcom_all_mte_memb/long_term_mean', fi_le))
        models = np.append(models, fi_le.split('.mte')[0])
print(models)
# kera
...
for _v in varbs:
    ...
    for plotVar, fig_num in var_info.items():
        ...
        cb.ax.set_xticklabels(cblabels_rat,
                            ...
                            ha='center',
                            rotation=0)
        plt.savefig(co_settings['fig_settings']['fig_dir'] + 'ztet_rs_'+_v+'_fig_' + fig_num + co_settings['exp_suffix'] + '.' +
                    fig_set['fig_format'],
                    bbox_inches='tight',
